...
function moreSlider() {
	if (!slidedMore) {
		...
		document.querySelector('.slider').style.animation = 'sliderFullIn 0.4s ease-in forwards';
		slidedMore = true;
	} else {
		document.querySelector('.slider').style.animation = '';
		document.querySelector('.slider').style.animation = 'sliderOut 0.4s ease-in forwards';
		slidedMore = false;
	}
}
